...
function seedYear() {
  mock.seed('users', { id: 'u1', email: 'test@example.com', username: 'testuser', password_hash: 'hash' });
  ...
  // Set created_at far in past so past-date logging works
  const pastDate = '2025-01-01T00:00:00Z';
  const hDaily = mock.seed('habits', { profile_id: PROFILE, title: 'Meditate', target_per_week: 7, created_at: pastDate });
  const hThree = mock.seed('habits', { profile_id: PROFILE, title: 'Gym', target_per_week: 3, created_at: pastDate });
  const hDead = mock.seed('habits', { profile_id: PROFILE, title: 'Old habit', target_per_week: 5, deleted_at: '2026-01-15T00:00:00Z', created_at: pastDate });
...
(async () => {
  ...
  section('Habit logging: duplicate, delete, range');
  {
    ...
    // Use a date outside the seeded year (365 days back) - e.g., 400 days ago
    const farPastDate = addDaysToDateString(today, -400);
    const r2 = await call(habitController.logCompletion, { params: { id: hThree.id }, body: { date: farPastDate } });
    check(r2.status === 201, 'logging a past date → 201', `got ${r2.status}`);
    const r3 = await call(habitController.deleteLog, { params: { id: hThree.id, date: farPastDate } });
    check(r3.status === 204, 'deleting that log → 204', `got ${r3.status}`);
    const r4 = await call(habitController.deleteLog, { params: { id: hThree.id, date: farPastDate } });
